...
public:
  chmin_add_max_x_plus_y(): M(0){}
  chmin_add_max_x_plus_y(int n, Val x0, Val y0): N(n), M(1 << ceil_pow2(n)), val(2 * M - 1){
    for(int i = 0; i < n; i++){
      val[M - 1 + i].maxx = x0;
      val[M - 1 + i].maxy = y0;
  ...
  chmin_add_max_x_plus_y(const std::vector<std::pair<Val, Val>> &v): N(v.size()), M(1 << ceil_pow2(N)), val(2 * M - 1){
...
int main(){
  ...
  int n, q;
  std::cin >> n >> q;
  chmin_add_max_x_plus_y<ll> seg(n, 0, 0);
  range(i, 0, q){
    int t;
    std::cin >> t;
    int l, r;
    ...
    if(t == 1){
      std::cin >> x;
      seg.update_add(l, r, x, 0);
    }else if(t == 2){
      std::cin >> x;
      seg.update_add(l, r, 0, x);
    }else if(t == 3){
      std::cin >> x;
      seg.update_chmin_x(l, r, x);
    }else if(t == 4){
      std::cin >> x;
      seg.update_chmin_y(l, r, x);
    }else{
      std::cout << seg.query_max(l, r) << '\n';
    }
  }
}
*/